 ...
 *
 ***********************************************************************/
static void print_hvqm_info(HVQM2Header *header) {
    int err;
            ...
            if (frame_format == HVQM2_VIDEO_HOLD) {
                ...
                /* If using the RSP version of the decoder */
                {
                    ...
                    status = hvqm2DecodeSP1(hvqbuf, frame_format, &cfb[bufno][screen_offset],
                                            ...
                                            hvq_spfifo);
#else
                    status = hvqm2DecodeSP2(hvqbuf, frame_format, &cfb[bufno][screen_offset],
                                            &cfb[prev_bufno][screen_offset], hvqwork, &hvq_sparg,
                                            hvq_spfifo);
#endif
...
#else
                    hvqm2Decode2(hvqbuf, frame_format, &cfb[bufno][screen_offset],
                                 &cfb[prev_bufno][screen_offset], hvqwork);
#endif
